...
def preprocess(x, y):
    ...
    first_nan_index = np.where(np.isnan(y))[0][0] if np.any(np.isnan(y)) else len(y)
    return x[:first_nan_index], y[:first_nan_index]
...
@jit(nopython=True)
def model_n(params, ISI, n, x, model):
    """
    ...
    tuple (list of np.arrays, np.array): 
        ...
        - combined response
    """
    # # Convert params to a Numba typed list
    # params_typed = List()
    # for p in params:
    #     params_typed.append(p)
    ...
    for i in range(n):
        delay = idx * i
        x_delayed = np.arange(0, len(x) * dt - delay * dt, dt)

        if model == 'alpha':
            b = params[-1]
            pars = (params[i], b)
            y_delayed = alpha_alt(pars, x_delayed)
        elif model == 'alpha2':
            b = params[n]
            d = params[-1]
            pars = (params[i], b, params[i + n + 1], d)
            y_delayed = alpha2_alt(pars, x_delayed)
        elif model == 'product':
            b = params[-2]
            c = params[-1]
            pars = (params[i], b, c)
            y_delayed = product_alt(pars, x_delayed)
        elif model == 'product2':
            b = params[n]
            c = params[n + 1]
            e = params[-2]
            f = params[-1]
            pars = (params[i], b, c, params[i + n + 2], e, f)
            y_delayed = product2_alt(pars, x_delayed)

        y_delayed_padded = np.zeros(len(x))
        y_delayed_padded[delay:delay + len(y_delayed)] = y_delayed

        individual_responses.append(y_delayed_padded)
        combined_response += y_delayed_padded
...
def nFIT(x, y, n=1, ISI=50, bl=0, model='product2', criterion='AIC', plot=True, initial_guess=None, fit_method='LS', percent=80, dp=3, cv=0.2, N=30, rise=[0.2, 0.8], decay=[0.8, 0.2], kinetics='solve', show_results=False, slow_rise_constraint=True, nll_calculation='ols'):
    """
    perform fitting of a specified model
    ...
    params_fit = List([p for p in params_fit])
...
def alpha2_output(params_fit, x, y, x_orig, y_orig, ids, n=5, ISI=50, bl=0, criterion='AIC', plot=True, dp=3, rise=[0.2, 0.8], decay=[0.8, 0.2], kinetics='solve', show_results=False, nll_calculation='ols'):
    ...
    out = List([par for par in out1] + [par for par in out2])        
...
def product2_output(params_fit, x, y, x_orig, y_orig, ids, n=5, ISI=50, bl=0, criterion='AIC', plot=True, dp=3, rise=[0.2, 0.8], decay=[0.8, 0.2], kinetics='solve', nll_calculation='ols', show_results=False):
